...
for y in ['log10sSFR','log10age']: #'log10age','age',
    ...
    for x, z in zip(x_, x_[::-1]): #the colour map is for the other parameter
        ...
        fig, ax = flares_utility.plt.linear_redshift(D, zeds, x, y, s[x], limits = limits, scatter_colour_quantity = z, rows=2)
        ...
        fig, ax = flares_utility.plt.linear_redshift(D, zeds, x, y, s[x], limits = limits, scatter_colour_quantity = z, rows=1)
