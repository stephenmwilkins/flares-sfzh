...
# for ax, tag, z in zip(axes, fl.tags[:1], fl.zeds):
    ...
    for log10Mstar_limit, ls in zip([8.5, 9.5, 10.5], ['-','--',':']):
        ...
        sfr, bin_edges = np.histogram(Age, weights = MassInitial*Weight, bins = np.arange(0.,1.5, 0.01))
        ...
        ax.plot(bin_centres, sfr, ls = ls, lw = 1.5,  c = c, label = rf'$\rm \log_{{10}}M^{{\star}}={log10Mstar_limit-0.5:.1f}-{log10Mstar_limit+0.5:.1f}$')
