...
# for ax, tag, z in zip(axes, fl.tags[:3], fl.zeds):
    ...
    for log10Mstar_limit, ls in zip([8.5, 9.5, 10.5], ['-','--',':']):
        ...
        dt = 0.01
        sfr, bin_edges = np.histogram(Age, weights = MassInitial*Weight, bins = np.arange(0.,1.5, dt))

        sfr /= np.sum(sfr)
        bin_centres = bin_edges[:-1]+0.5*(bin_edges[1:]-bin_edges[:-1])


        dsfr = -np.gradient(sfr, dt)/sfr

        ax.plot(bin_centres, dsfr, ls = '-', lw = 1.5,  c = c, label = rf'$\rm \log_{{10}}M^{{\star}}={log10Mstar_limit-0.5:.1f}-{log10Mstar_limit+0.5:.1f}$')

        # ddsfr = -np.gradient(dsfr, dt)#/dsfr
        #
        # ax.plot(bin_centres, ddsfr, ls = ':', lw = 1.5,  c = c, label = rf'$\rm \log_{{10}}M^{{\star}}={log10Mstar_limit-0.5:.1f}-{log10Mstar_limit+0.5:.1f}$')
...
axes[0].set_ylabel(rf'$\rm (dSFR/dt)/SFR$', fontsize = 9)
axes[3].set_ylabel(rf'$\rm (dSFR/dt)/SFR$', fontsize = 9)
